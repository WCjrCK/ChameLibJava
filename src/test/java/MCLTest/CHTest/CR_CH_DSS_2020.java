...
public class CR_CH_DSS_2020 extends BasicParam {
    ...
    static void initTest() {
        InitialLib();
        try {
            File_Writer = new BufferedWriter(new FileWriter("./data/MCL/CH/CR_CH_DSS_2020.txt"));
    ...
    void MCLG1Test(MCL curve) {
        Func.MCLInit(curve);
        {
            ...
            G1[] m = new G1[repeat_cnt];
            G1[] m2 = new G1[repeat_cnt];
            for (int i = 0; i < repeat_cnt; i++) {
                pk[i] = new MCL_G1.PublicKey();
                sk[i] = new MCL_G1.SecretKey();
                m[i] = new G1();
                m2[i] = new G1();
                pp.GP.GetGElement(m[i]);
                pp.GP.GetGElement(m2[i]);
            ...
            {
                long start = System.nanoTime();
                for(int i = 0;i < repeat_cnt;++i) scheme.Adapt(rp[i], h[i], r[i], pp, pk[i], sk[i], m[i], m2[i]);
                long end = System.nanoTime();
                double duration = (end - start) / 1.0e6;
                time_cost[++stage_id] = duration / repeat_cnt;
            }
    ...
    void MCLG2Test(MCL curve) {
        Func.MCLInit(curve);
        {
            ...
            G2[] m = new G2[repeat_cnt];
            G2[] m2 = new G2[repeat_cnt];
            for (int i = 0; i < repeat_cnt; i++) {
    ...
    void MCLGTTest(MCL curve) {
        Func.MCLInit(curve);
        {
            ...
            GT[] m = new GT[repeat_cnt];
            GT[] m2 = new GT[repeat_cnt];
            for (int i = 0; i < repeat_cnt; i++) {
