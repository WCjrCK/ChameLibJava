...
import ChameleonHash.CH.DEPRECATED.CH_CDK_2017.Native;
...
public class CH_CDK_2017 extends BasicParam {
    ...
    void NativeTest(int k) {
        ...
        BigInteger[] L2 = new BigInteger[repeat_cnt];
        ...
        {
            long start = System.nanoTime();
            for(int i = 0;i < repeat_cnt;++i) scheme.Adapt(rp[i], r[i], pk[i], sk[i], L[i], m[i], L2[i], m2[i]);
            long end = System.nanoTime();
            double duration = (end - start) / 1.0e6;
            time_cost[++stage_id] = duration / repeat_cnt;
        }
        ...
        {
            boolean res = true;
            for(int i = 0;i < repeat_cnt;++i) res &= scheme.Check(h[i], rp[i], pk[i], L2[i], m2[i]);
