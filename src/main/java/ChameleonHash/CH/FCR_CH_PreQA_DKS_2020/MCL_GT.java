package ChameleonHash.CH.FCR_CH_PreQA_DKS_2020;
...
public class MCL_GT {
    public static class PublicParam {
        ...
        public void H(Fr res, GT m1, GT m2, Fr m3, GT m4, GT m5) {
            H(res, String.format("(%s|%s|%s)(%s|%s)", m1, m2, m3, m4, m5));
        }

        public void H_p(GT res, GT m) {
    ...
    public void Hash(HashValue H, Randomness R, PublicParam pp, PublicKey pk, Fr m) {
        ...
        Mcl.pow(H.O, pp.g_1, m);
        Mcl.pow(G_tmp[0], pp.g_2, R.s_1_2);
        Mcl.mul(H.O, H.O, G_tmp[0]);

        pp.GP.GetZrElement(Fr_tmp[0]);

        pp.GP.GetZrElement(R.s_1_1);

        Mcl.pow(G_tmp[0], pp.g_1, Fr_tmp[0]);
        Mcl.pow(G_tmp[1], pp.g_2, R.s_1_1);
        Mcl.mul(G_tmp[0], G_tmp[0], G_tmp[1]);

        Mcl.pow(G_tmp[1], pp.g_1, R.s_2);
    ...
    public boolean Check(HashValue H, Randomness R, PublicParam pp, PublicKey pk, Fr m) {
        ...
        Mcl.pow(G_tmp[0], pp.g_1, R.s_1_1);
        Mcl.pow(G_tmp[1], pp.g_2, R.s_1_2);
        Mcl.mul(G_tmp[0], G_tmp[0], G_tmp[1]);
        Mcl.pow(G_tmp[1], H.O, R.e_1);
    ...
    public void Adapt(Randomness R_p, HashValue H, Randomness R,PublicParam pp, PublicKey pk, SecretKey sk, Fr m, Fr m_p) {
        ...
        Mcl.pow(G_tmp[0], pp.g_1, R_p.s_1_1);
        Mcl.pow(G_tmp[1], pp.g_2, R_p.s_1_2);
        Mcl.mul(G_tmp[0], G_tmp[0], G_tmp[1]);
        Mcl.pow(G_tmp[1], H.O, R_p.e_1);
        Mcl.inv(G_tmp[1], G_tmp[1]);
        Mcl.mul(G_tmp[0], G_tmp[0], G_tmp[1]);

        Mcl.pow(G_tmp[1], pp.g_1, Fr_tmp[0]);
