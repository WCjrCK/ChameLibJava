package ChameleonHash.PBCH.DEPRECATED.PCHBA_TLL_2020;
...
public class MCL {
    public static class PublicParam {
        public ABE.FAME.MCL.PublicParam pp_FAME = new ABE.FAME.MCL.PublicParam();
    ...
    public static class MasterPublicKey {
        ...
        public ABE.FAME.MCL.MasterPublicKey mpk_FAME = new ABE.FAME.MCL.MasterPublicKey();
        public G1 g_alpha = new G1();
        public G2 h_d_alpha = new G2(), h_1_alpha = new G2(), h_beta_alpha = new G2(), pk_ch = new G2();
        public G1[] g_i, g_alpha_i;
        public G2[] h_i;
    }

    public static class MasterSecretKey {
        public ABE.FAME.MCL.MasterSecretKey msk_FAME = new ABE.FAME.MCL.MasterSecretKey();
//        alpha, beta, z1~k
    ...
    public static class SecretKey {
        public ABE.FAME.MCL.SecretKey sk_FAME = new ABE.FAME.MCL.SecretKey();
        public G1[] sk_0_g = new G1[]{new G1(), new G1(), new G1()};
        public G1 sk_1 = new G1();
        public Fr sk_ch = new Fr();
        public G1[] sk_2;
        private final Fr delegate_z = new Fr();

        private final G1[] G1_tmp = new G1[]{new G1(), new G1(), new G1()};
        ...
        public boolean delegate(PublicParam SP, MasterPublicKey mpk, MasterSecretKey msk, G1 ID_i_1, Fr I_i_1) {
//            mod.ssk.sk_FAME, SP.pp_FAME, mpk.mpk_FAME, msk.msk_FAME, S, r_1, r_2, R
            ...
            Mcl.mul(G2_tmp[0], mpk.mpk_FAME.h, Fr_tmp[2]);
            Mcl.add(sk_FAME.sk_0[0], sk_FAME.sk_0[0], G2_tmp[0]);

            Mcl.div(Fr_tmp[4], Fr_tmp[2], msk.msk_FAME.a_2);
            Mcl.div(Fr_tmp[2], Fr_tmp[2], msk.msk_FAME.a_1);

            Mcl.mul(G2_tmp[0], mpk.mpk_FAME.h, Fr_tmp[3]);
            Mcl.add(sk_FAME.sk_0[1], sk_FAME.sk_0[1], G2_tmp[0]);

            Mcl.div(Fr_tmp[5], Fr_tmp[3], msk.msk_FAME.a_2);
            Mcl.div(Fr_tmp[3], Fr_tmp[3], msk.msk_FAME.a_1);

            Mcl.mul(G2_tmp[0], mpk.h_1_alpha, Fr_tmp[0]);
            Mcl.add(sk_FAME.sk_0[2], sk_FAME.sk_0[2], G2_tmp[0]);

            Mcl.mul(G1_tmp[0], sk_0_g[0], Fr_tmp[0]);
            Mcl.add(sk_0_g[1], sk_0_g[1], G1_tmp[0]);
            ...
            for(Map.Entry<String, Integer> entry : sk_FAME.Attr2id.entrySet()) {
                SP.pp_FAME.H(G1_tmp[0], entry.getKey() + "11");
                Mcl.mul(G1_tmp[0], G1_tmp[0], Fr_tmp[2]);

                SP.pp_FAME.H(G1_tmp[1], entry.getKey() + "21");
                Mcl.mul(G1_tmp[1], G1_tmp[1], Fr_tmp[3]);

                SP.pp_FAME.H(G1_tmp[2], entry.getKey() + "31");
                Mcl.mul(G1_tmp[2], G1_tmp[2], Fr_tmp[6]);

                Mcl.add(sk_FAME.sk_y[entry.getValue()][0], sk_FAME.sk_y[entry.getValue()][0], G1_tmp[0]);
                Mcl.add(sk_FAME.sk_y[entry.getValue()][0], sk_FAME.sk_y[entry.getValue()][0], G1_tmp[1]);
                Mcl.add(sk_FAME.sk_y[entry.getValue()][0], sk_FAME.sk_y[entry.getValue()][0], G1_tmp[2]);

                SP.pp_FAME.H(G1_tmp[0], entry.getKey() + "12");
                Mcl.mul(G1_tmp[0], G1_tmp[0], Fr_tmp[4]);

                SP.pp_FAME.H(G1_tmp[1], entry.getKey() + "22");
                Mcl.mul(G1_tmp[1], G1_tmp[1], Fr_tmp[5]);

                SP.pp_FAME.H(G1_tmp[2], entry.getKey() + "32");
                Mcl.mul(G1_tmp[2], G1_tmp[2], Fr_tmp[7]);

                Mcl.add(sk_FAME.sk_y[entry.getValue()][1], sk_FAME.sk_y[entry.getValue()][1], G1_tmp[0]);
                Mcl.add(sk_FAME.sk_y[entry.getValue()][1], sk_FAME.sk_y[entry.getValue()][1], G1_tmp[1]);
                Mcl.add(sk_FAME.sk_y[entry.getValue()][1], sk_FAME.sk_y[entry.getValue()][1], G1_tmp[2]);
            }

            SP.pp_FAME.H(G1_tmp[0], "0111");
            Mcl.mul(G1_tmp[0], G1_tmp[0], Fr_tmp[2]);

            SP.pp_FAME.H(G1_tmp[1], "0121");
            Mcl.mul(G1_tmp[1], G1_tmp[1], Fr_tmp[3]);

            SP.pp_FAME.H(G1_tmp[2], "0131");
            Mcl.mul(G1_tmp[2], G1_tmp[2], Fr_tmp[6]);

            Mcl.add(sk_FAME.sk_p[0], sk_FAME.sk_p[0], G1_tmp[0]);
            Mcl.add(sk_FAME.sk_p[0], sk_FAME.sk_p[0], G1_tmp[1]);
            Mcl.add(sk_FAME.sk_p[0], sk_FAME.sk_p[0], G1_tmp[2]);

            SP.pp_FAME.H(G1_tmp[0], "0112");
            Mcl.mul(G1_tmp[0], G1_tmp[0], Fr_tmp[4]);

            SP.pp_FAME.H(G1_tmp[1], "0122");
            Mcl.mul(G1_tmp[1], G1_tmp[1], Fr_tmp[5]);

            SP.pp_FAME.H(G1_tmp[2], "0132");
            Mcl.mul(G1_tmp[2], G1_tmp[2], Fr_tmp[7]);

            Mcl.add(sk_FAME.sk_p[1], sk_FAME.sk_p[1], G1_tmp[0]);
            Mcl.add(sk_FAME.sk_p[1], sk_FAME.sk_p[1], G1_tmp[1]);
            Mcl.add(sk_FAME.sk_p[1], sk_FAME.sk_p[1], G1_tmp[2]);

            Mcl.mul(G1_tmp[0], ID_i_1, Fr_tmp[0]);
            Mcl.mul(G1_tmp[1], sk_2[0], I_i_1);
            Mcl.add(sk_1, sk_1, G1_tmp[0]);
            Mcl.add(sk_1, sk_1, G1_tmp[1]);

            sk_2 = Arrays.copyOfRange(sk_2, 1, sk_2.length);

            Mcl.mul(G1_tmp[0], mpk.g_alpha_i[sk_2.length - 1], delegate_z);
            Mcl.add(sk_2[0], sk_2[0], G1_tmp[0]);
    ...
    public static class HashValue {
        G2 b = new G2(), h_p = new G2();
        Fr[] owner_ID;
    }

    public static class Randomness {
        public ABE.FAME.MCL.CipherText ct_FAME = new ABE.FAME.MCL.CipherText();
        Fr sigma = new Fr();
        G1 epk = new G1();
        G2 p = new G2(), ct_0_4 = new G2(), ct_1 = new G2(), ct_2 = new G2(), ct_3 = new G2(), c = new G2();
        byte[] ct, ct_p;
    }

    public static class User {
        private final SecretKey ssk = new SecretKey();
        public G1 ID_hat = new G1();
        public G2 ID_hat_h = new G2(), ID_hat_alpha = new G2();
        public Fr[] ID;
        ...
        public boolean delegate(PublicParam SP, MasterPublicKey mpk, MasterSecretKey msk, Fr I_i_1) {
            ...
            Mcl.mul(G1_tmp[0], mpk.g_i[mpk.g_i.length - ID.length], I_i_1);
            Mcl.add(ID_hat, ID_hat, G1_tmp[0]);

            Mcl.mul(G2_tmp[0], mpk.h_i[mpk.h_i.length - ID.length], I_i_1);
            Mcl.add(ID_hat_h, ID_hat_h, G2_tmp[0]);

            Mcl.mul(Fr_tmp[0], I_i_1, msk.alpha);

            Mcl.mul(G2_tmp[0], mpk.h_i[mpk.h_i.length - ID.length], Fr_tmp[0]);
            Mcl.add(ID_hat_alpha, ID_hat_alpha, G2_tmp[0]);

            Mcl.mul(G1_tmp[0], ID_hat, msk.alpha);

            return ssk.delegate(SP, mpk, msk, G1_tmp[0], I_i_1);
        }
    }

    ABE.FAME.MCL FAME = new ABE.FAME.MCL();
    ...
    public void SetUp(MasterPublicKey mpk, MasterSecretKey msk, int k) {
        ...
        mpk.g_alpha_i = new G1[k];
        for(int i = 0; i < k; ++i) {
            mpk.g_alpha_i[i] = new G1();
    ...
    public void AssignUser(User usr, MasterPublicKey mpk, MasterSecretKey msk) {
        ...
        for(int i = 0;i < usr.ID.length;++i) {
            Mcl.mul(G1_tmp[0], mpk.g_i[mpk.g_i.length - i - 1], usr.ID[i]);
            Mcl.add(usr.ID_hat, usr.ID_hat, G1_tmp[0]);
            Mcl.mul(G2_tmp[0], mpk.h_i[mpk.g_i.length - i - 1], usr.ID[i]);
            Mcl.add(usr.ID_hat_h, usr.ID_hat_h, G2_tmp[0]);
        }
    ...
    public void KeyGen(User mod, PublicParam SP, MasterPublicKey mpk, MasterSecretKey msk, BooleanFormulaParser.AttributeList S) {
        ...
        Mcl.mul(G1_tmp[0], mod.ID_hat, Fr_tmp[0]);
        Mcl.add(mod.ssk.sk_1, mod.ssk.sk_1, G1_tmp[0]);
        ...
        mod.ssk.sk_2 = new G1[mpk.g_i.length - mod.ID.length];
        for(int i = 0;i < mod.ssk.sk_2.length;++i) {
            mod.ssk.sk_2[i] = new G1();
    ...
    private void GenCipher(Randomness R, PublicParam SP, MasterPublicKey mpk, User owner, base.LSSS.MCL.Matrix MSP, Fr r, byte[] R_) {
        ...
        FAME.Encrypt(R.ct_FAME, SP.pp_FAME, mpk.mpk_FAME, MSP, new ABE.FAME.MCL.PlainText(GT_tmp[0]), Fr_tmp[0], Fr_tmp[1]);
        Mcl.add(Fr_tmp[0], Fr_tmp[0], Fr_tmp[1]);
        ...
        Mcl.pairing(GT_tmp[0], mpk.mpk_FAME.g, mpk.h_d_alpha);
    ...
    public boolean Check(HashValue H, Randomness R, PublicParam SP, MasterPublicKey mpk, Fr m) {
        Mcl.mul(G2_tmp[0], H.h_p, m);
        Mcl.add(G2_tmp[0], R.p, G2_tmp[0]);
        Mcl.pairing(GT_tmp[0], mpk.g_alpha, R.ct_2);
        Mcl.pow(GT_tmp[0], GT_tmp[0], R.sigma);

        Mcl.pairing(GT_tmp[1], R.epk, R.ct_1);
        Mcl.pairing(GT_tmp[2], mpk.mpk_FAME.g, R.ct_3);
        SP.H2(Fr_tmp[0], R.epk + "|" + R.c);
        Mcl.pow(GT_tmp[2], GT_tmp[2], Fr_tmp[0]);
        Mcl.mul(GT_tmp[1], GT_tmp[1], GT_tmp[2]);
        return H.b.equals(G2_tmp[0]) && GT_tmp[0].equals(GT_tmp[1]);
    }
    ...
    public void Adapt(Randomness R_p, HashValue H, Randomness R, PublicParam SP, MasterPublicKey mpk, MasterSecretKey msk, User moder, base.LSSS.MCL.Matrix MSP, Fr m, Fr m_p) {
        ...
        Mcl.pairing(GT_tmp[0], moder_p.ssk.sk_1, R.ct_FAME.ct_0[2]);
        Mcl.pairing(GT_tmp[1], moder_p.ssk.sk_0_g[1], R.ct_1);
        Mcl.pairing(GT_tmp[2], moder_p.ssk.sk_0_g[2], R.ct_0_4);
        Mcl.mul(GT_tmp[1], GT_tmp[1], GT_tmp[2]);
        ...
        ABE.FAME.MCL.PlainText pt_RABE = new ABE.FAME.MCL.PlainText();
