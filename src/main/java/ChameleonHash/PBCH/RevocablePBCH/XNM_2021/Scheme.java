...
//        dk.CHET_sk.CopyFrom(sk.CHET_sk);
    ...
    public void Collision(Randomness r_p, PublicParam pp, MasterPublicKey mpk, SecretKey sk, Message m, HashValue h, Randomness r, Message m_p) {
        ...
        Scalar u_1 = pp.H(Arrays.toString(rb) + "|" + h.RABE_ct.P.FAME_p.MSP.formula + "|" + Arrays.toString(pp.serializeInfo(new Info(sk.RABE_sk.dk.info))));
        Scalar u_2 = pp.H(Arrays.toString(pp.serializeInfo(new Info(sk.RABE_sk.dk.info))) + "|" + h.RABE_ct.P.FAME_p.MSP.formula + "|" + Arrays.toString(rb));

        CipherText RABE_ct = pp.RABE_pp.createCipherText();
        pp.RABE.Encrypt(RABE_ct, pp.RABE_pp, mpk.RABE_mpk, h.RABE_ct.P, RABE_pt, sk.RABE_sk.dk.info, u_1, u_2);
