...
public class MCL {
    ...
    public static class SecretKey {
        ...
        public HashMap<Integer, G1> sk_theta = new HashMap<>();
        int node_id;
    }
    ...
    public static class UpdateKey {
        ...
        public void AddKey(int theta, G1 k_u_theta_0, G2 k_u_theta_1) {
            G1 tmp_g1 = new G1();
            Mcl.neg(tmp_g1, k_u_theta_0);
            Mcl.neg(tmp_g1, tmp_g1);
            ku_theta_G1.put(theta, tmp_g1);
            G2 tmp_g2 = new G2();
            Mcl.neg(tmp_g2, k_u_theta_1);
            Mcl.neg(tmp_g2, tmp_g2);
            ku_theta_G2.put(theta, tmp_g2);
        }
    }
    ...
    public ABE.FAME.MCL FAME = new ABE.FAME.MCL();
    ...
    public void KeyGen(SecretKey sk, base.BinaryTree.MCL_G1 st, PublicParam SP, MasterPublicKey mpk, MasterSecretKey msk, BooleanFormulaParser.AttributeList S, G1 id) {
        ...
        Func.GetMCLG1RandomElement(sk.sk_FAME.sk_p[2]);
    }
    ...
    public void UpdateKeyGen(UpdateKey ku, PublicParam SP, MasterPublicKey mpk, base.BinaryTree.MCL_G1 st, base.BinaryTree.MCL_G1.RevokeList rl, int t) {
        ...
        for(int theta = 0;theta < st.g_theta.length;++theta) {
            if(st.tag.get(theta) && st.tag_g.get(theta)) {
                ...
                SP.H(G1_tmp[0], String.valueOf(t));
                Mcl.mul(G1_tmp[0], G1_tmp[0], Fr_tmp[0]);
                Mcl.add(G1_tmp[0], st.g_theta[theta], G1_tmp[0]);
                Mcl.mul(G2_tmp[0], mpk.mpk_FAME.h, Fr_tmp[0]);
                ku.AddKey(theta, G1_tmp[0], G2_tmp[0]);
            }
        }
    }
    ...
    public void DecryptKeyGen(DecryptKey dk, PublicParam SP, MasterPublicKey mpk, SecretKey sk, UpdateKey ku, base.BinaryTree.MCL_G1 st, base.BinaryTree.MCL_G1.RevokeList rl) {
        ...
        if(theta != -1) {
            ...
            G1 ku_theta_0 = ku.ku_theta_G1.get(theta);
            G2 ku_theta_1 = ku.ku_theta_G2.get(theta);
    ...
    public void Encrypt(CipherText CT, PublicParam SP, MasterPublicKey mpk, base.LSSS.MCL.Matrix MSP, PlainText PT, int t, Fr s_1, Fr s_2) {
        ...
        if(SP.type == TYPE.XNM_2021) {
            FAME.Encrypt(CT.ct_FAME, SP.pp_FAME, mpk.mpk_FAME, MSP, new ABE.FAME.MCL.PlainText(PT.m), s_1, s_2);
        } else if(SP.type == TYPE.TMM_2022) {
            Func.GetMCLGTRandomElement(GT_tmp[0]);
            Mcl.inv(GT_tmp[1], GT_tmp[0]);
            Mcl.mul(GT_tmp[0], GT_tmp[1], GT_tmp[0]);
            FAME.Encrypt(CT.ct_FAME, SP.pp_FAME, mpk.mpk_FAME, MSP, new ABE.FAME.MCL.PlainText(GT_tmp[0]), s_1, s_2);
    ...
    public void EncryptFr(CipherText CT, PublicParam SP, MasterPublicKey mpk, base.LSSS.MCL.Matrix MSP, PlainTextFr PT, int t) {
        Func.GetMCLZrRandomElement(Fr_tmp[1]);
        Func.GetMCLZrRandomElement(Fr_tmp[2]);
        EncryptFr(CT, SP, mpk, MSP, PT, t, Fr_tmp[1], Fr_tmp[2]);
    }

    public void EncryptFr(CipherText CT, PublicParam SP, MasterPublicKey mpk, base.LSSS.MCL.Matrix MSP, PlainTextFr PT, int t, Fr s_1, Fr s_2) {
        Mcl.add(Fr_tmp[0], s_1, s_2);
        SP.H(CT.ct_0_4, String.valueOf(t));
        Mcl.mul(CT.ct_0_4, CT.ct_0_4, Fr_tmp[0]);
        Func.GetMCLGTRandomElement(GT_tmp[0]);
    ...
    public void Decrypt(PlainText PT, PublicParam SP, DecryptKey dk, base.LSSS.MCL.Matrix MSP, CipherText CT) {
        ABE.FAME.MCL.PlainText pt_FAME = new ABE.FAME.MCL.PlainText();
        FAME.Decrypt(pt_FAME, MSP, CT.ct_FAME, dk.sk_FAME);
        Mcl.pairing(PT.m, CT.ct_0_4, dk.sk_0_4);
    ...
    public void DecryptFr(PlainTextFr PT, DecryptKey dk, base.LSSS.MCL.Matrix MSP, CipherText CT) {
        ABE.FAME.MCL.PlainText pt_FAME = new ABE.FAME.MCL.PlainText();
        FAME.Decrypt(pt_FAME, MSP, CT.ct_FAME, dk.sk_FAME);
        Mcl.pairing(GT_tmp[0], CT.ct_0_4, dk.sk_0_4);
