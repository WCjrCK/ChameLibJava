...
public class MCL_G2 {
    public static class RevokeList {
        ...
        public void Add(G2 id, int expire_time) {
    ...
    public MCL_G2(int n) {
        g_theta = new G2[2 * n - 1];
        for (int i = 0; i < g_theta.length; i++) g_theta[i] = new G2();
    ...
    public void Setg(int node_id) {
        tag_g.set(node_id);
        Func.GetMCLG2RandomElement(g_theta[node_id]);
    }

    public void Setg(int node_id, G2 g) {
        tag_g.set(node_id);
